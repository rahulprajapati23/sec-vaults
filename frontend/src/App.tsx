...
function DashboardHome() {
  const { data: filesData } = useQuery({
    ...
    queryFn: () => api.get('/files').then(r => r.data.files as any[]),
    staleTime: 30000,
  });

  const { data: eventsData } = useQuery({
    queryKey: ['dashboard-events'],
    queryFn: () => api.get('/dam/events?limit=200').then(r => r.data.events as any[]),
    staleTime: 30000,
  ...
  const totalSize = files.reduce((acc: number, f: any) => acc + (f.size_bytes || 0), 0);
  const threats = events.filter((e: any) => e.severity === 'high' || e.severity === 'critical').length;
  const cleanFiles = files.filter((f: any) => f.scan_status !== 'infected').length;
